...
# 构建完成后会连接ssh, 执行/node/sh/node.sh, node.sh会将构建的完成资源复制到/node/xxx, 并且执行/node/xxx/deploy/nuxt-pm2.sh
# 最后, 服务器的/node/sh/node.sh会执行清除buff/cache操作

# nuxt项目, 一般流程是在jenkins里面执行nuxt-build.sh进行构建,
# 构建完成后会连接ssh, 执行/node/sh/nuxt.sh, nuxt.sh会将构建的完成资源复制到/node/xxx, 并且执行/node/xxx/deploy/nuxt-pm2.sh
...
if [ $ENV != 'null' ]; then
    ...
    echo "执行$PUBLICDIR/$JOBNAME/$ENV/deploy/nuxt-pm2.sh"
    sh $PUBLICDIR/$JOBNAME/$ENV/deploy/nuxt-pm2.sh $JOBNAME $ENV $WORKSPACE $PORT $TAG
else
    echo 当前环境是null
    ...
    echo "执行$PUBLICDIR/$JOBNAME/$ENV/deploy/nuxt-pm2.sh"
    sh $PUBLICDIR/$JOBNAME/deploy/nuxt-pm2.sh $JOBNAME $ENV $WORKSPACE $PORT $TAG
fi
